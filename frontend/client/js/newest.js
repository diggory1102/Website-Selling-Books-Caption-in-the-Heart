document.addEventListener('DOMContentLoaded', async () => {
    ...
    let currentSortParam = 'newest'; // Bắt buộc sort newest
    const priceParam = urlParams.get('price');
    const pageParam = parseInt(urlParams.get('page')) || 1;

    // Hàm cập nhật URL khi người dùng thay đổi bộ lọc
    const applyFilters = () => {
        let newUrl = `newest.html?`;
    ...
    try {
        ...
        let sidebarHtml = `<li><a href="newest.html" class="${!categoryId ? 'active' : ''}">Tất cả truyện</a></li>`;
        
        categories.forEach(cat => {
            const catId = cat.id || cat._id;
            const isActive = (catId === categoryId) ? 'active' : '';
            sidebarHtml += `<li><a href="newest.html?id=${catId}" class="${isActive}"><i class="fa-solid fa-angle-right" style="font-size: 10px; margin-right: 5px;"></i> ${cat.name}</a></li>`;
            
            // Cập nhật tiêu đề trang nếu khớp ID
            if (isActive) {
                catTitle.textContent = `Thể loại Mới: ${cat.name}`;
